% Model the average temperature of the Mars.
...
Atm.Height_atm = 11e3; % [m] Total height of athmosphere considered
Atm.Rair = 188.9; % [J/kg/K] CO2 specific gas constant = R/mu
Atm.N = 10; % Number of segments in the athmosphere
Atm.dh = Atm.Height_atm/Atm.N; % [m] Height of one segment of athmosphere

Atm.P0 = 610; % [Pa] % Standard pressure at the ground level on Mars
Atm.I_Sun = 125; % [W/m^2] Flux coming from the Sun
Atm.alfa_sun = 0.16; % Reflected by the athmosphere/clouds etc
Atm.alfa_N = 0.02; % The earth’s surface (snow,ice,and so on) reflects a fraction alfa_N of what it receives
Atm.g = 3.721; % [m/s^2] on Mars

Atm.sigVis = 0; % [m^-1] No absorbtion in visible light on Mars
Atm.sigIR = 0.0021131; % [m^-1] % The larger the coeff the higher the absorbtion. 0 means no absorbtion

Atm.Bolt = 5.670374419184429453970e-8 ; %[W/m^2K^4]


T0 = 300*ones(1,Atm.N+1); % Initial values for iterations
